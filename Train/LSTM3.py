...
def build_model(model_path, weight_path, lstm_config, dense_config, time_steps=1):
    ...
    if os.path.exists(model_path):
        ...
        sum_lstm_model.add(LSTM(output_dim=100, activation=lstm_config['activation'],
                                recurrent_activation=lstm_config['recurrent_activation'], return_sequences=True))
        ...
        date_model.add(
            Dense(input_shape=(dense_config['date_features_shape'][0], dense_config['date_features_shape'][1]),
                  units=dense_config['date_features_shape'][1], activation='relu'))
        # date_model.add(Dropout(0.5))
